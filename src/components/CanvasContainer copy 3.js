...
function CanvasContainer() {
  ...
  const resetCamera = () => {
    const originalCameraZ = 20; // Default Z position of the camera
    const newDragX = 0; // Set dragX back to the initial position
  
    animateCameraAndDrag(newDragX, originalCameraZ);
  };
  ...
  const handleModelClick = (index) => {
    ...
    const leftmostPositionX = -5; // Desired X position to bring the model to the left side
    ...
    // Smoothly animate dragX and camera movement
    animateCameraAndDrag(newDragX, targetCameraZ);
